...
async def process_course_content(message: Message, state: FSMContext):
    ...
    new_lesson_number = len(courses) + 1
    ...
    db.add_lesson(new_lesson_number, new_lesson)
    await message.answer(f"Курс '{title}' успешно создан и добавлен как урок номер {new_lesson_number}.")
    await state.clear()
